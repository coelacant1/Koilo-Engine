...
struct KSL_SpectrumAnalyzerShader : public ksl::Shader {
    int gradientCount = 2;
    ...
    KSL_PARAMS_BEGIN(KSL_SpectrumAnalyzerShader)
        ...
        KSL_PARAM(float, heightScale, "Height multiplier",  0.1f, 10.0f)
        KSL_PARAM(bool,  mirrorY,     "Mirror Y axis",      0, 1)
        KSL_PARAM(bool,  flipY,       "Flip Y axis",        0, 1)
    KSL_PARAMS_END
    ...
    ksl::vec4 shade(const ksl::ShadeInput& in) const override {
        ...
        float fx = ksl::map(rPos.x, -sizeHalfX, sizeHalfX, float(binCount), 0.0f);
        ...
        float height = ksl::cosineInterpolation(s0, s1, t) * heightScale;

        float yColor = 0.0f;
        if (mirrorY) {
            yColor = ksl::map(ksl::abs(rPos.y), sizeHalfY, 0.0f, 1.0f, 0.0f);
        } else {
            yColor = ksl::map(rPos.y, -sizeHalfY, sizeHalfY, 1.0f, 0.0f);
        }
