 ...
 *
 * The MeshAlign class is useful for positioning, scaling, and orienting 3D objects
...
private:
     ...
     *
     * @param objs Array of pointers to Mesh instances.
     ...
     */
    void NormalizeObjectPlane(Mesh** objs, uint8_t numObjects, Vector3D center, Quaternion planeOrientation);
     ...
     */
    void NormalizeObjectCenter(Mesh** objs, uint8_t numObjects, Vector3D center);

public:
    /**
     * @brief Constructor for MeshAlign, setting up the camera bounds and orientation.
     ...
     */
    MeshAlign(Vector2D camMin, Vector2D camMax, Quaternion targetOrientation = Quaternion());
     ...
     */
    Vector3D GetObjectCenter(Mesh* obj);

    /**
     * @brief Computes the collective center for multiple objects.
     * @param objs Array of pointers to Mesh.
     * @param numObjects Number of objects in \p objs.
     * @return A Vector3D representing the combined center of all objects.
     */
    Vector3D GetObjectCenter(Mesh** objs, uint8_t numObjects);
     ...
     */
    Vector3D GetObjectSize(Mesh* obj);
     ...
     */
    Vector3D GetObjectSize(Mesh** objs, uint8_t numObjects);
     ...
     */
    Quaternion GetPlaneNormal(Mesh* obj);
     ...
     */
    Quaternion GetPlaneNormal(Mesh** objs, uint8_t numObjects);
     ...
     */
    Quaternion GetPlaneOrientation(Mesh* obj, Vector3D centroid);
     ...
     */
    Quaternion GetPlaneOrientation(Mesh** objs, uint8_t numObjects, Vector3D centroid);

    /**
     * @brief Computes the final Transform for aligning a single Mesh within the camera bounds.
     *
     * The Transform includes translation, rotation, and scaling as determined by the justification,
     * margins, and plane offset angle.
     *
     * @param obj Pointer to the Mesh to align.
     * @return A Transform struct containing position, rotation, and scale.
     */
    Transform GetTransform(Mesh* obj);
     ...
     */
    Transform GetTransform(Mesh** objs, uint8_t numObjects);
     ...
     */
    float GetObjectPlanarityRatio(Mesh* obj);
     ...
     */
    float GetObjectPlanarityRatio(Mesh** objs, uint8_t numObjects);
    ...
    /**
     * @brief Aligns a single Mesh within the camera bounds without applying the object's scaling factor.
     *
     * Only transforms the object to fit in the plane (translation + orientation); no scaling from the object itself.
     * @param obj Pointer to the Mesh to align.
     */
    void AlignObjectNoScale(Mesh* obj);
     ...
     * @param numObjects Number of objects in \p objs.
     */
    void AlignObjectsNoScale(Mesh** objs, uint8_t numObjects);

    /**
     * @brief Aligns a single Mesh within the camera bounds, including applying scale factors as necessary.
     * @param obj Pointer to the Mesh to align.
     */
    void AlignObject(Mesh* obj);
