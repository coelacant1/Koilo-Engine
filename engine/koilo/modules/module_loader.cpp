...
// Adapter that wraps C ABI function pointers as an IEngineModule.
class DynamicModuleAdapter : public IEngineModule {
public:
    ...
    bool Initialize(scripting::KoiloScriptEngine* engine) override {
        ...
        services_.register_global = [](void* eng, const char* name, const char* cls, void* inst) {
            static_cast<scripting::KoiloScriptEngine*>(eng)->RegisterGlobal(name, cls, inst);
        };
        services_.get_global   = nullptr;  // TODO: implement
        services_.set_variable = [](void* eng, const char* name, double value) {
            static_cast<scripting::KoiloScriptEngine*>(eng)->SetScriptVariable(name, scripting::Value(value));
        };
        services_.get_variable = [](void* eng, const char* name) -> double {
            auto val = static_cast<scripting::KoiloScriptEngine*>(eng)->GetScriptVariable(name);
            return (val.type == scripting::Value::Type::NUMBER) ? val.numberValue : 0.0;
        };
        ...
        return initFunc_(&services_, engine) != 0;
    }
...
bool ModuleLoader::TryLoad(const std::string& name) {
    ...
    for (auto& path : candidates) {
        if (LoadFromLibrary(path)) {
            ...
            if (engine_ && !modules_.empty()) {
                auto& m = modules_.back();
                if (m->Initialize(engine_)) {
...
bool ModuleLoader::ReloadModule(const std::string& name) {
    ...
    for (size_t i = 0; i < modules_.size(); ++i) {
        if (modules_[i]->GetInfo().name == name) {
            ...
            // Load fresh copy
            if (LoadFromLibrary(libPath) && engine_ && !modules_.empty()) {
                auto& m = modules_.back();
                if (m->Initialize(engine_)) {
                    initialized_.push_back(m.get());
                    return true;
